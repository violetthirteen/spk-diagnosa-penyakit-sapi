@extends('layouts.app')

@section('title-section', 'Hasil Diagnosa')

@section('content')

@php
    $topPenyakit = array_key_first($hasil);
    $topPersen = $topPenyakit ? $hasil[$topPenyakit]['persen'] : 0;
@endphp

<div style="max-width:780px;margin:0 auto;">

    <div style="background:linear-gradient(135deg,#1e3a8a,#2563eb);border-radius:16px 16px 0 0;padding:28px 32px;text-align:center;">
        <h1 style="font-size:20px;font-weight:800;color:white;letter-spacing:.3px;">
            LAPORAN HASIL DIAGNOSA
        </h1>
        <p style="color:#93c5fd;font-size:13px;margin-top:4px;font-weight:500;">
            Sistem Pakar Diagnosa Penyakit Pada Sapi
        </p>
    </div>

    <div style="background:white;border-left:1px solid #e2e8f0;border-right:1px solid #e2e8f0;padding:20px 32px;">
        <table style="width:100%;border-collapse:collapse;font-size:14px;">
            <tr>
                <td style="color:#64748b;width:50%;padding:4px 0;">Diagnosa Oleh</td>
                <td style="color:#1e293b;font-weight:600;padding:4px 0;">{{ Auth::user()->name }}</td>
            </tr>
            <tr>
                <td style="color:#64748b;padding:4px 0;">Tanggal</td>
                <td style="color:#1e293b;font-weight:600;padding:4px 0;">{{ now()->timezone('Asia/Jakarta')->format('d/m/Y H:i') }} WIB</td>
            </tr>
        </table>
    </div>

    @if($topPenyakit && $topPersen < 50)
    <div style="background:#fffbeb;border-left:1px solid #f59e0b;border-right:1px solid #f59e0b;padding:14px 32px;text-align:center;">
        <p style="color:#92400e;font-weight:600;font-size:13px;">
            Hasil diagnosa belum cukup kuat ({{ number_format($topPersen, 2) }}%). Disarankan melakukan diagnosa ulang dengan gejala yang lebih spesifik.
        </p>
        <a href="/diagnosa" class="btn btn-warning" style="margin-top:8px;color:white;font-size:13px;padding:8px 16px;">Diagnosa Ulang</a>
    </div>
    @endif

    <div style="background:white;border-left:1px solid #e2e8f0;border-right:1px solid #e2e8f0;padding:24px 32px;">

        {{-- GEJALA YANG DIPILIH --}}
        <div style="margin-bottom:24px;">
            <div style="font-size:13px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:.5px;margin-bottom:10px;border-bottom:2px solid #6366f1;padding-bottom:8px;">
                Gejala yang Dipilih
            </div>
            <div style="display:flex;flex-wrap:wrap;gap:6px;">
                @foreach($selectedGejala as $g)
                    <span class="badge badge-primary">{{ $g->kode_gejala }} - {{ $g->nama_gejala }}</span>
                @endforeach
            </div>
        </div>

        @if($hasil)

        {{-- HASIL ANALISIS --}}
        <div style="margin-bottom:24px;">
            <div style="font-size:13px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:.5px;margin-bottom:14px;border-bottom:2px solid #2563eb;padding-bottom:8px;">
                Hasil Analisis Penyakit
            </div>

            @foreach($hasil as $penyakit => $data)
                @php
                    $isFirst = $loop->first;
                    $persen = $data['persen'];
                    $barColor = $isFirst ? '#2563eb' : ($loop->index == 1 ? '#f59e0b' : '#94a3b8');
                @endphp

                <div style="margin-bottom:{{ $loop->last ? '0' : '12px' }};padding:{{ $isFirst ? '16px' : '12px 16px' }};background:{{ $isFirst ? '#f8faff' : '#fafafa' }};border-radius:8px;border:{{ $isFirst ? '1.5px solid #2563eb' : '1px solid #e2e8f0' }};">

                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
                        <div style="flex:1;">
                            @if($isFirst)
                                <div style="font-size:11px;color:#2563eb;font-weight:700;text-transform:uppercase;letter-spacing:.5px;margin-bottom:2px;">
                                    Diagnosis Utama
                                </div>
                            @endif
                            <div style="font-weight:700;color:#0f172a;font-size:{{ $isFirst ? '15px' : '14px' }};">
                                {{ $penyakit }}
                            </div>
                        </div>
                        <div style="font-size:{{ $isFirst ? '20px' : '15px' }};font-weight:800;color:{{ $barColor }};">
                            {{ number_format($persen, 2) }}%
                        </div>
                    </div>

                    <div style="background:#e2e8f0;border-radius:999px;height:{{ $isFirst ? '8px' : '6px' }};overflow:hidden;">
                        <div style="height:100%;width:{{ min($persen, 100) }}%;background:{{ $barColor }};border-radius:999px;"></div>
                    </div>

                </div>
            @endforeach
        </div>

        {{-- TENTANG PENYAKIT --}}
        @if($topPenyakit && isset($hasil[$topPenyakit]['deskripsi']))
        <div style="margin-bottom:24px;">
            <div style="font-size:13px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:.5px;margin-bottom:10px;border-bottom:2px solid #f59e0b;padding-bottom:8px;">
                Tentang {{ $topPenyakit }}
            </div>
            <p style="color:#475569;font-size:14px;line-height:1.8;text-align:justify;">
                {{ $hasil[$topPenyakit]['deskripsi'] }}
            </p>
        </div>
        @endif

        {{-- SOLUSI --}}
        @if($topPenyakit && isset($hasil[$topPenyakit]['solusi']))
        <div style="margin-bottom:24px;">
            <div style="font-size:13px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:.5px;margin-bottom:10px;border-bottom:2px solid #059669;padding-bottom:8px;">
                Solusi Penanganan
            </div>

            @php
                $solusiList = \App\Models\Aturan::whereHas('penyakit', fn($q) => $q->where('nama_penyakit', $topPenyakit))
                    ->with('solusi')
                    ->get()
                    ->pluck('solusi')
                    ->unique('id');
            @endphp

            @foreach($solusiList as $solusi)
            <div style="display:flex;gap:12px;padding:12px 14px;background:#f8fafc;border-radius:8px;margin-bottom:8px;border:1px solid #e2e8f0;">
                <div style="width:4px;background:#059669;border-radius:2px;flex-shrink:0;"></div>
                <div>
                    <div style="color:#475569;font-size:14px;line-height:1.6;">
                        {{ $solusi->deskripsi_solusi }}
                    </div>
                </div>
            </div>
            @endforeach
        </div>
        @endif

        {{-- KESIMPULAN --}}
        <div style="margin-bottom:24px;">
            <div style="font-size:13px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:.5px;margin-bottom:10px;border-bottom:2px solid #6366f1;padding-bottom:8px;">
                Kesimpulan
            </div>

            <div style="padding:16px;background:#f8faff;border-radius:8px;border:1px solid #e0e7ff;">
                @if($topPersen < 50)
                <p style="color:#475569;font-size:14px;line-height:1.7;">
                    Berdasarkan gejala yang dipilih, sistem mendeteksi kemungkinan
                    <strong>{{ $topPenyakit }}</strong> dengan tingkat keyakinan
                    <strong>{{ number_format($topPersen, 2) }}%</strong>.
                    Namun, hasil ini belum cukup kuat untuk dijadikan kesimpulan akhir.
                    Disarankan untuk berkonsultasi dengan dokter hewan dan melakukan
                    diagnosa ulang dengan gejala yang lebih lengkap dan spesifik.
                </p>
                @else
                <p style="color:#475569;font-size:14px;line-height:1.7;">
                    Berdasarkan gejala yang dipilih, sistem mendeteksi bahwa sapi
                    terindikasi menderita <strong>{{ $topPenyakit }}</strong>
                    dengan tingkat keyakinan <strong>{{ number_format($topPersen, 2) }}%</strong>.
                    Disarankan untuk segera melakukan penanganan sesuai solusi yang
                    telah diberikan dan berkonsultasi dengan dokter hewan untuk
                    penanganan lebih lanjut.
                </p>
                @endif
            </div>
        </div>

        @else
        <div style="text-align:center;padding:40px 20px;">
            <div style="width:60px;height:60px;background:#f1f5f9;border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 12px;font-size:28px;color:#94a3b8;">?</div>
            <h3 style="color:#334155;font-size:16px;margin-bottom:6px;">Tidak Ada Hasil</h3>
            <p style="color:#64748b;font-size:14px;">Tidak ada penyakit yang cocok dengan gejala yang dipilih.</p>
            <a href="/diagnosa" class="btn btn-primary" style="margin-top:16px;">Coba Lagi</a>
        </div>
        @endif

    </div>

    <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:0 0 16px 16px;padding:16px 32px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;">
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
            <a href="/diagnosa" class="btn btn-outline" style="font-size:13px;padding:8px 16px;">Diagnosa Ulang</a>
            <a href="/riwayat-diagnosa" class="btn btn-outline" style="font-size:13px;padding:8px 16px;border-color:#64748b;color:#64748b;">Riwayat Diagnosa</a>
        </div>
        @if($topPenyakit)
        <a href="javascript:void(0)" onclick="window.print()" style="display:inline-flex;align-items:center;gap:6px;padding:8px 16px;background:#2563eb;color:white;border-radius:8px;font-size:13px;font-weight:600;text-decoration:none;cursor:pointer;border:none;">
            Cetak
        </a>
        @endif
    </div>

</div>

<style media="print">
    .sidebar, footer, .topbar, .alert, .menu-toggle, .topbar-title, noscript{display:none !important;}
    .main-wrapper{margin-left:0 !important;padding:0 !important;}
    .main-content{padding:0 !important;}
    body{background:white !important;}
    body::before, body::after{display:none !important;}
    .btn-outline, .btn-primary, .btn{display:none !important;}
    .main-content a[onclick]{display:inline-flex !important;}
    @page{margin:15mm;}
</style>

@endsection
